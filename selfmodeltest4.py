...
def block(n_output1,n_output2, upscale=False):
    
    def f(x):
        
        
       
        h = Conv2D(kernel_size=3, filters=n_output1, strides=1, padding='same')(x)
        ...
        h = BatchNormalization()(h)
        h = Activation('relu')(h)
        h = Conv2D(kernel_size=3, filters=n_output2, strides=1, padding='same')(x)
        ...
        if upscale:
            # 1x1 conv2d
            f = Conv2D(kernel_size=1, filters=n_output2, strides=1, padding='same')(x)
        else:
            # identity
            f = x
...
x=Conv2D(64,kernel_size=(7,7),strides=(1,1))(input_tensor)
x=BatchNormalization()(x)
x=Activation('relu')(x)
x=MaxPooling2D(pool_size=(3,3))(x)
x = block(64,128,upscale=True)(x)

x = block(128,256,upscale=True)(x)

x = block(256,512,upscale=True)(x)

x = block(512,1024,upscale=True)(x)

x = block(1024,2048,upscale=True)(x)
