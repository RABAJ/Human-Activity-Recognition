...
bone_list=[[21,9],[9,10],[10,11],[11,12],[12,24],[12,25]]
bone_list=np.array(bone_list)-1
from PIL import Image

import os
bone_list=[[2,12],[12,13],[13,14]]
...
#     ax.set(xlim=(-1,2),ylim=(-2,2),zlim=(-1,0.75));
    ...
    if(os.path.exists(os.path.join('/home/ws2/Downloads/Florence_3d_actions','Florence_dataset_WorldCoordinates.txt'))):
        with open(os.path.join('/home/ws2/Downloads/Florence_3d_actions','Florence_dataset_WorldCoordinates.txt'),'r') as f:
            for line in f:
                words=line.split()
                if(os.path.exists('/home/ws2/Documents/flore_rightleg/'+'a'+words[2]+'s'+words[0]+'p'+words[1]+'.png')):
                    print('continue')
                    continue
...
#                     return
                    print(int(words[2]))
#                     plt.show()
                    plt.savefig('/home/ws2/Documents/flore_rightleg/'+'a'+words[2]+'s'+words[0]+'p'+words[1]+'.png',dpi=100)
                    croppingimage('/home/ws2/Documents/flore_rightleg/'+'a'+words[2]+'s'+words[0]+'p'+words[1]+'.png')
#                     return
                    z1=0
                    fig=plt.figure()
                    fig.set_size_inches(68.5,60.5)
