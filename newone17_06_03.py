...
model=Sequential()
model.add(Conv3D(32,kernel_size=(3,3,3),input_shape=(64,64,10,3),strides=(1,1,1)))
...
model.add(Conv3D(128,kernel_size=(3,3,3),strides=(1,1,1)))
model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(MaxPooling3D(pool_size=(2,2,2)))
...
def fit_model(model, dataset1,onehot_encoded1,dataset,onehot_encoded):
    history = model.fit(dataset,onehot_encoded,
                        ...
                        validation_steps=len(dataset1) //32,
